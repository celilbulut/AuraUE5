											  ...
											  FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
	...
	// 1 ile 100 arasinda random bir block sansi olusturuyoruz.
	const bool bBlock = FMath::RandRange(1, 100) < TargetBlockChance;
	
	// If Block, have the damage.
	Damage = bBlock ? Damage / 2.f : Damage; // Alttaki kod ile ayni sey egitmen bunu kullandi.
	// if (bBlock) { Damage = Damage / 2.f; }
	...
	// Critical Hit Resistance calculation
	...
	const FRealCurve* CriticalHitResistanceCurve = CharacterClassInfo->DamageCalculationCoefficients->FindCurve(FName("Critical Hit Resistance"), FString());
